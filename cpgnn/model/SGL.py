...
class SGL(object):
    ...
    pretrain_embedding:np.array) -> None:
        ...
        """ Build model for code smell detection """
        with tf.device("/device:GPU:" + args.gpu_id[1]):
            if self.smell_train:
                self._build_smell_model()
                self._build_smell_loss()
    ...
    pretrain_embedding:np.array) -> None:
        ...
        self.smell_train = getattr(args, 'smell_test_supervised', False)
    ...
    def _build_inputs(self) -> None:
        ...
        # input function/label for code smell detection
        self.smell_funcs = tf.placeholder(dtype=tf.int64, name='smell_funcs',
            shape=[None, self.max_entity_func])
        self.smell_labels = tf.placeholder(dtype=tf.int64, name='smell_labels',
            shape=[None])
    ...
    def _build_smell_model(self) -> None:
        """Build model for code smell detection"""
        # Get function representations
        self.smell_func_rep = self._rep4funcs(self.smell_funcs)
        
        # Binary classification for smell detection
        with tf.variable_scope("smell_classification"):
            # Dense layer for classification
            self.smell_logits = tf.layers.dense(
                self.smell_func_rep, 
                units=2,  # Binary classification: smell vs no-smell
                activation=None,
                name="smell_classifier"
            )
            
            # Predictions
            self.smell_prediction = tf.nn.softmax(self.smell_logits, name="smell_prediction")
            self.smell_predicted_labels = tf.argmax(self.smell_prediction, axis=1, name="smell_predicted_labels")

    def _build_smell_loss(self) -> None:
        """Build loss function for code smell detection"""
        # Convert labels to one-hot encoding
        smell_labels_onehot = tf.one_hot(self.smell_labels, depth=2)
        
        # Cross-entropy loss
        self.smell_loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(
                labels=smell_labels_onehot,
                logits=self.smell_logits
            )
        )
        ...
        if self.regs[0] > 0:
            ...
            for param in tf.trainable_variables():
                if 'smell_classifier' in param.name:
                    reg_loss += tf.nn.l2_loss(param)
            self.smell_loss += self.regs[0] * reg_loss
        
        # Optimizer
        self.smell_opt = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(self.smell_loss)

    def train_smell(self, sess: tf.Session, feed_dict: dict) -> Tuple[None, float]:
        """Train code smell detection model"""
        return sess.run([self.smell_opt, self.smell_loss], feed_dict)

    def eval_smell(self, sess: tf.Session, feed_dict: dict) -> Tuple[np.array, np.array]:
        """Evaluate code smell detection model"""
        predictions, predicted_labels = sess.run([self.smell_prediction, self.smell_predicted_labels], feed_dict)
        return predictions, predicted_labels
    
    def _rep4funcs(self, funcs):
        """ Obtain function representation for code clone detection """
        # funcs: [batch_size, max_entity_func]
        # entity_embedding: [n_entity + 1, inter_dim]
        # f_rep: [batch_size, inter_dim]
        f_rep = tf.nn.embedding_lookup(self.g_embedding, funcs)
        f_rep = tf.nn.l2_normalize(f_rep, 1)

        # max pooling entity embeddings in a function
        # f_rep: [batch_size, inter_dim]
        f_rep = tf.layers.average_pooling1d(
            inputs=f_rep,
            pool_size=self.max_entity_func, strides=1,
            name='f_rep')
        f_rep = tf.reshape(f_rep, [-1, self.inter_dim])
        f_rep = tf.nn.l2_normalize(f_rep, 1)
