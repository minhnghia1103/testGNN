...
class GNNLoader(Data):
    def __init__(self, args:argparse.Namespace) -> None:
        ...
        # split functions into training/validation/testing sets for code smell detection
        if hasattr(args, 'smell_test_supervised') and args.smell_test_supervised:
            log.info('Generating code smell training, validation, and testing sets')
            self.smell_test_size = args.smell_test_size
            self.smell_val_size = args.smell_val_size
            self.batch_size_smell = args.batch_size_smell
            self._generate_code_smell_split()
            log.info('Code Smell [n_train, n_val, n_test] = [%d, %d, %d]' % (len(self.smell_train_data), len(self.smell_val_data), len(self.smell_test_data)))
    ...
    def _generate_code_smell_split(self):
        """Generate training/validation/test splits for code smell detection"""
        # Create labels for functions: 1 for smell, 0 for no-smell
        # You'll need to provide the actual labels based on your dataset
        func_labels = self._load_code_smell_labels()
        
        # Split functions with labels into train/val/test
        from sklearn.model_selection import train_test_split
        
        # First split: train+val vs test
        train_val_data, test_data = train_test_split(
            func_labels, test_size=self.smell_test_size, 
            random_state=42, stratify=[label for _, label in func_labels]
        )
        
        # Second split: train vs val
        train_data, val_data = train_test_split(
            train_val_data, test_size=self.smell_val_size/(1-self.smell_test_size),
            random_state=42, stratify=[label for _, label in train_val_data]
        )
        
        self.smell_train_data = train_data
        self.smell_val_data = val_data
        self.smell_test_data = test_data
        
        self.n_smell_train = len(self.smell_train_data)
        self.n_smell_val = len(self.smell_val_data)
        self.n_smell_test = len(self.smell_test_data)
        
        # batch iterator
        self.smell_data_iter = self.n_smell_train // self.batch_size_smell
        if self.n_smell_train % self.batch_size_smell:
            self.smell_data_iter += 1
            
        log.info(f'Code smell dataset split: train={self.n_smell_train}, val={self.n_smell_val}, test={self.n_smell_test}')

    def _load_code_smell_labels(self):
        """Load code smell labels from CSV file created by prepare_code_smell_data.py"""
        import csv
        import os
        
        # Try to load from CSV file created by prepare_code_smell_data.py
        label_file = os.path.join(os.path.dirname(__file__), '../code_smell_labels.csv')
        
        if os.path.exists(label_file):
            log.info(f"Loading code smell labels from {label_file}")
            func_labels = []
            ...
            with open(label_file, 'r', encoding='utf-8') as csvfile:
                ...
                for row in reader:
                    ...
                    has_smell = int(row['has_smell'])
                    func_labels.append([function_id, has_smell])
            
            log.info(f"Loaded {len(func_labels)} code smell labels from file")
            
            # Print distribution
            smell_count = sum(1 for _, label in func_labels if label == 1)
            total_count = len(func_labels)
            smell_ratio = smell_count / total_count if total_count > 0 else 0
            log.info(f"Dataset distribution: {smell_count}/{total_count} = {smell_ratio:.2%} smell")
            
        else:
            log.warning(f"Code smell label file not found at {label_file}")
            log.warning("Please run: python prepare_code_smell_data.py --input_folder /path/to/your/data")
            log.warning("Generating minimal synthetic labels for demo...")
            
            # Generate minimal synthetic labels for demonstration
            func_labels = []
            for func_id in range(min(self.n_entity, 100)):  # Only 100 samples for demo
                # Simple pattern: every 3rd function has smell
                label = 1 if func_id % 3 == 0 else 0
                func_labels.append([func_id, label])
            
            log.info(f"Generated {len(func_labels)} synthetic code smell labels for demo")
            
        return func_labels
    
    def generate_smell_train_batch(self, i_batch: int) -> dict:
        """Generate training batch for code smell detection"""
        batch_data = {}
        
        start = i_batch * self.batch_size_smell
        end = min((i_batch + 1) * self.batch_size_smell, self.n_smell_train)
        
        batch_funcs = self.smell_train_data[start:end]
        
        batch_data['smell_funcs'] = [func for func, _ in batch_funcs]
        batch_data['smell_labels'] = [label for _, label in batch_funcs]
        
        return batch_data

    def generate_smell_train_feed_dict(self, model, batch_data: dict) -> dict:
        """Generate feed dictionary for code smell training"""
        feed_dict = {
            model.smell_funcs: batch_data['smell_funcs'],
            model.smell_labels: batch_data['smell_labels'],
            # hardcode dropping probability for training
            model.mess_dropout: self.mess_dropout,
        }
        return feed_dict

    def generate_smell_val_batch(self, i_batch: int) -> dict:
        """Generate validation batch for code smell detection"""
        batch_data = {}
        
        start = i_batch * self.batch_size_smell
        end = min((i_batch + 1) * self.batch_size_smell, self.n_smell_val)
        
        batch_funcs = self.smell_val_data[start:end]
    ...
    def generate_smell_val_feed_dict(self, model, batch_data: dict) -> dict:
        """Generate feed dictionary for code smell validation"""
        feed_dict = {
            model.smell_funcs: batch_data['smell_funcs'],
            model.smell_labels: batch_data['smell_labels'],
            # hardcode dropping probability as 0 for validation
            model.mess_dropout: [0,0,0,0,0,0],
        }
        return feed_dict

    def generate_smell_test_batch(self, i_batch: int) -> dict:
        """Generate test batch for code smell detection"""
        batch_data = {}
        
        start = i_batch * self.batch_size_smell
        end = min((i_batch + 1) * self.batch_size_smell, self.n_smell_test)
        
        batch_funcs = self.smell_test_data[start:end]
    ...
    def generate_smell_test_feed_dict(self, model, batch_data: dict) -> dict:
        """Generate feed dictionary for code smell testing"""
        feed_dict = {
            model.smell_funcs: batch_data['smell_funcs'],
            model.smell_labels: batch_data['smell_labels'],
            # hardcode dropping probability as 0 for testing
            model.mess_dropout: [0,0,0,0,0,0],
        }
        return feed_dict
