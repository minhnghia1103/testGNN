...
def code_smell_val_supervised(sess: tf.Session, model, data_generator: GNNLoader, threshold: float) -> dict:
    """Validation function for code smell detection"""
    rel = {'valid': 1, 'recall': 0., 'precision': 0., 'f1': 0., 'accuracy': 0., 'auc': 0.}
    
    if data_generator.n_smell_val == 0:
        rel['valid'] = 0
        log.warn("n_smell_val == 0")
        return rel
    
    # Calculate number of validation batches
    n_batch_smell_val = data_generator.n_smell_val // data_generator.batch_size_smell
    if n_batch_smell_val == 0:
        n_batch_smell_val = 1
    elif data_generator.n_smell_val % data_generator.batch_size_smell:
        n_batch_smell_val += 1
    
    all_predictions = []
    all_labels = []
    
    for i_batch in range(n_batch_smell_val):
        batch_data = data_generator.generate_smell_val_batch(i_batch)
        feed_dict = data_generator.generate_smell_val_feed_dict(model, batch_data)
        
        predictions, predicted_labels = model.eval_smell(sess, feed_dict)
        
        all_predictions.extend(predictions[:, 1])  # Get probability of smell class
        all_labels.extend(batch_data['smell_labels'])
